...
type WritingSetDetail = any;

const PARTS = [
  { key: "part1_sentence", label: "Part 1 — Sentence (5)" },
  { key: "part2_email", label: "Part 2 — Email (2)" },
  { key: "part3_essay", label: "Part 3 — Essay (1)" },
] as const;
...
export default function AdminPracticeWritingSetDetailPage() {
  ...
  const [activePart, setActivePart] = useState<(typeof PARTS)[number]["key"]>("part1_sentence");
  const [rowKeyword, setRowKeyword] = useState("");

  async function load() {
    setLoading(true);
    try {
      const res: any = await apiClient.admin.skillTasks.getWritingSet(setId);
  ...
  async function loadBank() {
    setBankLoading(true);
    try {
      const res: any = await apiClient.admin.skillTasks.listWriting({ page: 1, limit: 100 });
  ...
  async function publish() {
    ...
    setSaving(true);
    try {
      await apiClient.admin.skillTasks.updateWritingSet(setId, { status: "published" });
  ...
  const bankOptions = useMemo(() => {
    ...
    return filtered.map((t) => ({ id: t.id, label: `${t.title} (${t.code})` }));
  }, [bank, activePart]);
  ...
  async function addSelected() {
    ...
    setSaving(true);
    try {
      await apiClient.admin.skillTasks.addWritingSetItems(setId, { taskIds: [selectedTaskId] });
  ...
  async function removeItem(itemId: string) {
    setSaving(true);
    try {
      await apiClient.admin.skillTasks.removeWritingSetItem(setId, itemId);
    ...
    <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="w-full space-y-4">
      <section className="surface p-4">
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => router.push("/admin/practice/writing")} className="btn-secondary">
            <ArrowLeft className="h-4 w-4" />
          </button>
          <h2 className="truncate text-lg font-semibold text-slate-900 admin-dark:text-[var(--admin-text)]">
            {setDetail?.title ?? "Bộ đề TOEIC Writing"}
          </h2>
        </div>
