  ...
  admin = {
    ...
    shadowing: {
      importYoutube: (data: {
        ...
        this.request('/admin/shadowing/import-youtube', {
          method: 'POST',
          body: JSON.stringify(data),
        }),
      ...
      list: (params?: {
        ...
        const qs = Object.keys(query).length ? `?${new URLSearchParams(query).toString()}` : '';
        return this.request(`/admin/shadowing${qs}`, { method: 'GET' });
      },

      getDetail: (id: string): Promise<ApiResponse> =>
        this.request(`/admin/shadowing/${encodeURIComponent(id)}`, { method: 'GET' }),

      update: (
        id: string,
        data: { title?: string; level?: string; topics?: string[]; status?: string },
      ): Promise<ApiResponse> =>
        this.request(`/admin/shadowing/${encodeURIComponent(id)}`, {
          method: 'PATCH',
          body: JSON.stringify(data),
        }),
      ...
      replaceSegments: (
        ...
        this.request(`/admin/shadowing/${encodeURIComponent(id)}/segments`, {
          method: 'PUT',
          body: JSON.stringify(data),
        }),

      publish: (id: string): Promise<ApiResponse> =>
        this.request(`/admin/shadowing/${encodeURIComponent(id)}/publish`, { method: 'POST' }),

      reimportEnglish: (id: string): Promise<ApiResponse> =>
        this.request(`/admin/shadowing/${encodeURIComponent(id)}/reimport-en`, { method: 'POST' }),

      translate: (id: string): Promise<ApiResponse> =>
        this.request(`/admin/shadowing/${encodeURIComponent(id)}/translate`, { method: 'POST' }),
  ...
  learner = {
    ...
    shadowing: {
      list: (params?: {
        ...
        const qs = Object.keys(query).length ? `?${new URLSearchParams(query).toString()}` : '';
        return this.request(`/learner/shadowing${qs}`, { method: 'GET' });
      },
